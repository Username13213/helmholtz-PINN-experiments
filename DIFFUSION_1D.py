...
def compute_pde_residuals(model, x, k):
...
def compute_total_loss(model, x_coll, x_l, x_r, lambda_bc=20.0):
    res_p, res_q = compute_pde_residuals(model, x_coll, k)
...
x_r = torch.tensor([[L]], device=device).requires_grad_(True)
u_exact, R_coeff, T_coeff = analytical_solution(x_np, k, n_d, x_a, x_b)

# Training ======================================================================
model = SIREN(layers=[1, 256, 256, 256, 256, 2], w0_start=float(k)).to(device)
...
for ep in pbar:
    optimizer.zero_grad()
    loss, lp, lb = compute_total_loss(model, x_coll, x_l, x_r, k)
    loss.backward()
...
opt_lb = torch.optim.LBFGS(
    model.parameters(), lr=1.0, max_iter=10_000,
    history_size=100, line_search_fn="strong_wolfe"
)

def closure():
    opt_lb.zero_grad()
    loss, _, _ = compute_total_loss(model, x_coll, x_l, x_r)
    loss.backward()
    return loss
...
u_total_pred = u_inc_np + u_scat_pred
...
ax.axvspan(x_a, x_b, **slab_kw)
ax.set_title("Absolute error |u_PINN − u_exact|")
ax.set_xlabel("x")
...
plt.tight_layout()
plt.savefig("scattering_1d_result.png", dpi=150, bbox_inches='tight')
plt.show()